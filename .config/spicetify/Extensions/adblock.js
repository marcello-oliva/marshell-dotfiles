...
const loadWebpack = () => {
    try {
      const o = window.webpackChunkclient_web.push([[Symbol()], {}, (n) => n]),
        ...
        r = new Set(Object.values(o.m)),
        c = i.flatMap((n) =>
          typeof n == "function"
            ? [n]
            : typeof n == "object" && n
              ? Object.values(n).filter(
                  (p) => typeof p == "function" && !r.has(p),
                )
              : [],
        );
  ...
  getSettingsClient = (o, s = [], i = {}) => {
    try {
      const r = o.find((c) => c?.settingsClient)?.settingsClient;
      if (!r) {
        const c = s.find(
          (n) =>
            n?.SERVICE_ID === "spotify.ads.esperanto.settings.proto.Settings" ||
            n?.SERVICE_ID === "spotify.ads.esperanto.proto.Settings",
        );
        return new c(i);
      }
      return r;
      ...
      return (
        console.error("adblockify: Failed to get ads settings client", r),
        null
      );
    }
  },
  getSlotsClient = (o, s) => {
    try {
      const i = o.find(
        (r) =>
          r.SERVICE_ID === "spotify.ads.esperanto.slots.proto.Slots" ||
          r.SERVICE_ID === "spotify.ads.esperanto.proto.Slots",
      );
      return new i(s);
    } catch (i) {
      return (console.error("adblockify: Failed to get slots client", i), null);
    }
  },
  getTestingClient = (o, s) => {
    try {
      const i = o.find(
        (r) =>
          r.SERVICE_ID === "spotify.ads.esperanto.testing.proto.Testing" ||
          r.SERVICE_ID === "spotify.ads.esperanto.proto.Testing",
      );
      return new i(s);
    } catch (i) {
      return (
        console.error("adblockify: Failed to get testing client", i),
        null
      );
    }
  },
  map = new Map(),
  retryCounter = (o, s) => {
    if ((map.has(o) || map.set(o, { count: 0 }), s === "increment"))
      map.get(o).count++;
    else if (s === "clear") map.delete(o);
    else if (s === "get") return map.get(o)?.count;
  };
(async function o() {
  (await new Promise((e) => Spicetify.Events.platformLoaded.on(e)),
    await new Promise((e) => Spicetify.Events.webpackLoaded.on(e)));
  const s = loadWebpack(),
    { Platform: i, Locale: r } = Spicetify,
    { AdManagers: c } = i;
  if (!c?.audio || Object.keys(c).length === 0) {
    setTimeout(o, 100);
    return;
  }
  const { audio: n } = c,
    ...
    u =
      p._product_state ||
      p._product_state_service ||
      i?.ProductStateAPI?.productStateApi,
    y = Spicetify.Platform.version.split(".").map((e) => Number.parseInt(e));
  ...
  if (g) f = (await g.getSlots()).adSlots;
  else
    try {
      f = await b.get("sp://ads/v1/slots");
    } catch {
      setTimeout(o, 100);
      return;
    }
  const C = () => {
    ...
    S = async () => {
      try {
        const { billboard: e, leaderboard: t, sponsoredPlaylist: a } = c,
          l = getTestingClient(s.functionModules, u.transport);
        if (
          (l
            ? l.addPlaytime({ seconds: -1e11 })
            : await b.post("sp://ads/v1/testing/playtime", { value: -1e11 }),
          await n.disable(),
          (n.isNewAdsNpvEnabled = !1),
          await e.disable(),
          await t?.disableLeaderboard(),
          await a.disable(),
          c?.inStreamApi)
        ) {
          const { inStreamApi: d } = c;
          await d.disable();
        }
        if (c?.vto) {
          const { vto: d } = c;
    ...
    k = async () => {
      for (const e of f)
        (E(e.slotId || e.slot_id),
          setTimeout(
            () => m({ adSlotEvent: { slotId: e.slotId || e.slot_id } }),
            50,
          ));
    },
    m = (e) => {
      ...
      try {
        const a = n?.inStreamApi?.adsCoreConnector;
        typeof a?.clearSlot == "function" && a.clearSlot(t);
...
Slot id: ${t}.`),
    ...
    w = async (e) => {
      try {
        ...
        if (!t) return;
        const a = y[0] === 1 && y[1] >= 2 && y[2] >= 82 ? 0n : "0";
        (await t.updateAdServerEndpoint({
          slotIds: [e],
          url: "http://localhost/no/thanks",
        }),
          await t.updateStreamTimeInterval({ slotId: e, timeInterval: a }),
          await t.updateSlotEnabled({ slotId: e, enabled: !1 }),
          await t.updateDisplayTimeInterval({ slotId: e, timeInterval: a }));
      } catch (t) {
        console.error(
    ...
    M = async () => {
      for (const e of f) w(e.slotId || e.slot_id);
    },
    E = (e) => {
      try {
        n.inStreamApi.adsCoreConnector.subscribeToSlot(e, m);
      } catch (t) {
    ...
    _ = () => {
        ...
        t = new MutationObserver((a) => {
          for (const l of a)
            if (l.addedNodes.length) {
              ...
              const h = d.querySelector("iframe");
              if (!h) continue;
              const A = h?.contentWindow?.document.body;
              if (
                !A ||
                !A.querySelector(
      ...
      for (const a of e) t.observe(a, { childList: !0 });
    },
    v = async () => {
      try {
        const e = JSON.parse(
        ...
        if (Spicetify?.RemoteConfigResolver) {
          const a = Spicetify.createInternalMap(t);
          Spicetify.RemoteConfigResolver.value.setOverrides(a);
        } else if (Spicetify.Platform?.RemoteConfigDebugAPI) {
          const a = Spicetify.Platform.RemoteConfigDebugAPI;
          for (const [l, d] of Object.entries(t))
            await a.setOverride({ source: "web", type: "boolean", name: l }, d);
        }
      } catch (e) {
        console.error(
